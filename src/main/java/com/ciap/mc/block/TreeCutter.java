...
public class TreeCutter extends OrientableBlock implements BlockEntityProvider {
    public static Identifier ID = getIdentifier("tree_cutter");

    public TreeCutter(Settings settings) {
        super(settings);
    }
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof TreeCutterBlockEntity) {
                ItemScatterer.spawn(world, pos, (TreeCutterBlockEntity)blockEntity);
    ...
    private void resetDestroyedBlock(World world, BlockPos pos) {
        ...
        world.setBlockBreakingInfo(((TreeCutterBlockEntity)entity).breakerId, targetBlockPos, -1);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntityRegistry.TREE_CUTTER, TreeCutterBlockEntity::tick);
    }
}
